...
public class Roulette extends  Casino {
    ...
    private void shouldFailSafeBeUsed(int numberChosen) {
        if (amountsOfTimesHouseLost > 3 || payout < casinoBalance) {
    ...
    public int rolleTheRoulette(int moneyPutIn, int numberChosen) {
        ...
        if (winningNumber == numberChosen) {
            casinoBalance -= payout; //als in dit geval casinoBalance minder dan payout
            playerBalance += payout; // en palayer balance meer dan payout
            return payout;  //dan krijgt de player 500 Euro
        } else {
            casinoBalance += payout;//als casinobalance meer dan payout
            playerBalance -= payout;//en playerbalance minder dan payout
